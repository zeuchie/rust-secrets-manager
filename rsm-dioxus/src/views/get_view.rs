            ...
            div { class: "max-w-2xl",
                    ...
                    match storage::load_vault_from_file() {
                        Ok(mut v) => {
                            ...
                            if let Some(secret) = maybe.cloned() {
                                ...
                                // clear any previous displayed values
                ...
                { if !username().is_empty() {
                    rsx!( div { class: "bg-slate-800 rounded-lg p-6 border border-slate-700",
                        ...
                        div { class: "space-y-4",
                            div { label { class: "text-slate-400 text-sm block mb-2", "Username" } div { class: "flex items-center gap-2", input { r#type: "text", value: "{username}", readonly: true, class: "flex-1 bg-slate-700 text-white px-3 py-2 rounded border border-slate-600" } button { onclick: move |_| status.set("Username copied (demo)".to_string()), class: "p-2 text-slate-400", Copy {color: "white", size: 18,}, } } }
